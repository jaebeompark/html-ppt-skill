...
## 1. card-news-editorial — white-ground magazine editorial
...
- **Path:** `templates/full-decks/card-news-editorial/index.html`
...
## 5. obsidian-claude-gradient — GitHub purple gradient
...
## 7. card-news-pastel — soft macaron, slow living
...
- **Path:** `templates/full-decks/card-news-pastel/index.html`

## 8. dir-key-nav-minimal — arrow-key, 8 colour minimal
...
| 12 | `weekly-report`    | 7  | Corporate clarity, 8-cell KPI grid, shipped list, 8-week bar chart, next-week table | Weekly status update, business review |
| 13 | `card-news-post`         | 9  | **3:4 @ 810×1080**, warm pastel, dashed sticker cards, page dots | Card-news post, Instagram carousel |
| 14 | `course-module`    | 7  | Warm paper + Playfair serif, persistent left sidebar of learning objectives, MCQ self-check | Teaching module, online course, workshop unit |
| 15 | `presenter-reveal` 🎤 | 8  | **Presenter-mode template** · Korean typography (`word-break:keep-all`) · Pretendard · a 150–300 character speaker script per slide plus `[리허설]` timing notes | Talks, internal tech sharing, lectures — anywhere you press `S` for the script ✨ |

Each folder: `index.html`, scoped `style.css` (prefixed `.tpl-<name>`), `README.md`. The `card-news-post` template overrides the default `.slide` box to fixed `810×1080` for 3:4 portrait.

> 🎤 **For any spoken presentation — a talk, a lecture, a roadshow — start from `presenter-reveal`**, or follow [presenter-mode.md](./presenter-mode.md) to add `<aside class="notes">` speaker scripts to another template.
