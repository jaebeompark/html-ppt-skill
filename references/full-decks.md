...
| 16 | `ko-presenter-reveal` 🇰🇷 | 8  | **한국어 발표자 모드 전용** · `word-break:keep-all` 한글 조판 · Pretendard · 매 슬라이드 150–300자 逐字稿 + `[리허설]` 시간 배분 메모 | **한국어 사내 기술 공유 / 팀 회고 / 발표 리허설** ✨ |
...
>
> 🇰🇷 **한국어 발표라면 `ko-presenter-reveal`** — 같은 발표자 모드에 한글 줄바꿈(`word-break:keep-all`)과 한국어 예시 逐字稿가 들어 있습니다.
